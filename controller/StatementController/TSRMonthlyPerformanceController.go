...
// @Success 200 {object} StatementControllerResponseSchema.TSRMonthlyPerformanceResponseSchema
// @Failure 500 {object} app.Response
// @Router /projectData/GetTSRMonthlyPerformanceData [Get]
func TSRMonthlyPerformanceDataController(c *gin.Context) {
	appG := app.Gin{C: c}
	...
	//签单月拨打表现
	TSRMonthlyPerformanceArray, err := StatementService.GetTSRMonthlyPerformanceBaseDataArray(PageNationRequest)
	if err != nil {
		appG.Response(http.StatusBadRequest, e.ERROR_EXIST_CALCULATION_TSR_GRADE, nil)
	} else {
		appG.Response(http.StatusOK, e.SUCCESS, TSRMonthlyPerformanceArray)
	}
}
